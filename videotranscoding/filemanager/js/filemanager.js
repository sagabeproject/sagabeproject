...
var fmLib =  {
	...
	call: function(url, frmName) {
		...
		ajaxObj.encoding = fmContSettings[curCont].encoding;
	...
	checkFile: function(listLoad, sid) {
		...
		if(iFrame) {
			...
			if(response.match(/end:1\}$/)) {
				...
				url.match(/fmContainer=(\w+)/);
				this.viewResponse(response, RegExp.$1, listLoad);
				...
				var url = fmWebPath + '/cgi/ping.pl?sid=' + sid;
				var ajaxObj = new ajax();
	...
	openDialog: function(url, dialogId, text, fileId, name, perms, x, y) {
		var f, e, i, obj, sid, curCont, iframeCont, ids, dialog, perc, dir, width, height;
					...
					switch(dialogId) {
						...
						case 'fmNewFile':
							...
							if(fmContSettings[curCont] && fmContSettings[curCont].perlEnabled) {
								...
								f.action = fmWebPath + '/cgi/upload.pl?cont=' + curCont + '&sid=' + sid;
							}
							else {
								f.action = url;
								sid = '';
		...
		if(dialogId == 'fmMediaPlayer' && typeof FlashReplace == 'object') {
			if(text[0].match(/\.swf$/i)) {
				...
				var params = {FlashVars: 'flvToPlay=' + escape(url) + '&defaultVolume=75&showScaleModes=false', bgcolor: '#000000', allowFullScreen: 'true'};
	...
	fileInfo: function(id, lang, curCont) {
	...
	viewMenu: function(id, lang, curCont) {
		if(this.noMenu) {
			this.noMenu = false;
			return;
		}
		var params, caption, html;
		...
		if(fmEntries[curCont][id].deleted) {
			...
			if(fmEntries[curCont][id].icon.substr(0, 3) == 'dir') {
				...
				items.push({icon:'download.gif',width:12,caption:fmMsg[lang].cmdGetFile,exec:['fmLib.getFile',curCont,id]});
			}
			else if(!userPerms.hideDisabledIcons) {
				items.push({icon:'download_x.gif',width:12,caption:fmMsg[lang].cmdGetFile});
			}
			items.push({icon:'info.gif',caption:fmMsg[lang].cmdFileInfo,exec:['fmLib.fileInfo',id,lang,curCont]});
			items.push({caption:'separator'});

			if(fmEntries[curCont][id].icon.match(/^(acrobat|word|excel|powerpoint|text)/i)) {
				if(userPerms.docViewer) {
					...
					else if(fmContSettings[curCont].publicUrl != '') {
			...
			else if(fmEntries[curCont][id].name.match(/\.(jpe?g|png|gif)$/i)) {
		...
		this.openDialog(null, 'fmMenu', [fmEntries[curCont][id].name, html]);
	},
