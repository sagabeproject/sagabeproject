...
/* PUBLIC METHODS ****************************************************************************** */
		...
		if(is_file($path)) {
			...
			if($disp != 'inline') {
				...
				if($this->FileManager->mailOnDownload) {
					$this->FileManager->sendDownloadInfo($this->path, $this->size);
				}
	...
	function setProperties($file, $dir) {
		...
		switch($sysType) {
			...
			case 'PHP':
				$filename = FM_Tools::basename($file, $this->FileManager->encoding);
...
/* PRIVATE METHODS ***************************************************************************** */
	...
	function _viewHeader() {
		...
		return "{id:'$this->id',deleted:$deleted";
	}
	 ...
	 */
	function _viewName() {
		$name = addslashes($this->isDeleted()? preg_replace('/\.' . FM_EXT_DELETED . '$/', '', $this->name) : $this->name);
		$path = substr($this->path, strlen($this->FileManager->startDir), strlen($this->path));
		$dir = addslashes(preg_replace('%^/$%', '', FM_Tools::dirname($path, $this->FileManager->encoding)));
